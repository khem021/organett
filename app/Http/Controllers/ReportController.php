...
class ReportController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        // Check if this period is enabled (farm admins bypass the restriction)
        if (! auth()->user()->isFarmAdmin() && ! auth()->user()->isSuperAdmin()) {
        ...
        $callback = function () use ($rows, $columns, $farmName, $label, $period) {
            ...
            // ── Report header ────────────────────────────────
            fputcsv($out, [$this->csvSafe($farmName)]);
    ...
    {
        if (in_array(mb_substr($value, 0, 1), ['=', '+', '-', '@', "\t", "\r"])) {
            return "'".$value;
        }
    ...
    private function dailyRows(): array
    {
        ...
        $data = Sale::selectRaw('DATE(sale_date) as day, SUM(amount) as revenue, COUNT(*) as cnt')
    ...
    {
        // DATE() works on both SQLite and MySQL; aggregate per week in PHP
        $windowStart = now()->startOfWeek()->subWeeks(11)->toDateString();
        $windowEnd = now()->endOfWeek()->toDateString();
        $byDay = Sale::selectRaw('DATE(sale_date) as day, SUM(amount) as revenue, COUNT(*) as cnt')
            ->whereBetween('sale_date', [$windowStart, $windowEnd])
            ->groupBy('day')
            ->get()
            ->keyBy('day');
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            for ($d = $start->copy(); $d->lte($end); $d->addDay()) {
                $revenue += (float) ($byDay[$d->toDateString()]->revenue ?? 0);
                $count += (int) ($byDay[$d->toDateString()]->cnt ?? 0);
            }
    ...
    private function monthlyRows(): array
    {
        ...
        $byDay = Sale::selectRaw('DATE(sale_date) as day, SUM(amount) as revenue, COUNT(*) as cnt')
            ...
            ->groupBy('day')
            ->get()
            ->groupBy(fn ($r) => substr($r->day, 0, 7)); // group by Y-m in PHP
    ...
    private function yearlyRows(): array
    {
        $data = Sale::selectRaw('DATE(sale_date) as day, SUM(amount) as revenue, COUNT(*) as cnt')
            ...
            ->orderBy('day')
            ->get()
            ->groupBy(fn ($r) => substr($r->day, 0, 4)); // group by year in PHP
        ...
        $rows = [];
        foreach ($data->sortKeys() as $year => $group) {
            $revenue = (float) $group->sum('revenue');
            $count = (int) $group->sum('cnt');
            $avg = $count > 0 ? $revenue / $count : 0;
            $rows[] = [
